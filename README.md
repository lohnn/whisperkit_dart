...
A Dart FFI wrapper for [WhisperKit](https://github.com/argentlabs/WhisperKit),
enabling high-performance speech-to-text on macOS using CoreML.

## Prerequisites

- **macOS**: This wrapper relies on WhisperKit, which is optimized for Apple
  Silicon and macOS.
- **Xcode**: Required to build the native Swift library (automatically handled
  by the Dart hook).
...
2. **Get Dart dependencies**:
   ...
   dart pub get
   ```
   _Note: This will automatically build the native WhisperKit library._
...
- `-f, --format`: Output format.
  ...
  - `clean_verbose_json`: Optimized JSON for storage (cleaned text, word
    timestamps).
- `-w, --word-timestamps`: Enable word-level timestamps (useful for
  `verbose_json`).
...
For podcast applications, we recommend using the **`clean_verbose_json`**
format. This format is optimized for storage and usability.
...
- **`segments`**: The core data for syncing and navigation.
  ...
  - `text`: The **cleaned** transcript text (stripped of metadata tags like
    `<|0.00|>`).
  - `words`: Word-level timestamps (`word`, `start`, `end`).

**Fields Discarded (to save space):**

- **Top-level `text`**: Redundant. Reconstruct the full transcript by joining
  the cleaned segment texts.
- **`timings`**, **`tokens`**, **`tokenLogProbs`**: Unnecessary internal
  metrics.
